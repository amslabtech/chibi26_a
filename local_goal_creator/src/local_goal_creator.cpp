...
void LocalGoalCreator::pathCallback(const nav_msgs::msg::Path::SharedPtr msg)//subのコールバック関数
{
    ...
    is_finished_ = false; // ★ここを追加！新しい走行を開始するためにリセット
    RCLCPP_INFO(this->get_logger(), "New path received. Size: %zu", path_.poses.size());
}
...
void LocalGoalCreator::process()//main文ので実行する関数
{
    ...
    {
        if (is_finished_) {
            // 完全に停止させるため、現在の自己位置をゴールとして送り続ける（偏差を0にする）
            geometry_msgs::msg::PointStamped stop_signal;
            stop_signal.header.frame_id = "map";
            stop_signal.header.stamp = this->now();
            stop_signal.point = pose_.pose.position; // 現在地をゴールに設定
            local_goal_pub_->publish(stop_signal);
        } else {
            publishGoal();
        }
    }
...
void LocalGoalCreator::publishGoal()
{
    ...
    // ###### 追加#########
    // if (goal_index_ >= (int)path_.poses.size() - 1) {
    //     if (dist_to_goal < finish_tolerance_) {
    //         is_finished_ = true;
    //         RCLCPP_INFO(this->get_logger(), "All waypoints completed. Stopping robot.");
    //         return; // ゴールを配信せずに終了
    //     }
    // }
    bool is_near_path_end = (goal_index_ >= (int)path_.poses.size() - 5); 

    // 2. ある程度走行が進んでいるか（例：全パスの半分以上を通過したか）
    // これにより、スタート直後にゴール判定されるのを防ぐ
    bool has_progressed = (goal_index_ > (int)path_.poses.size() / 2);

    if (is_near_path_end && has_progressed) {
        if (dist_to_goal < finish_tolerance_) {
            is_finished_ = true;
            RCLCPP_INFO(this->get_logger(), "Goal reached! Lap completed.");
            return;
        }
    }
    // ######################
