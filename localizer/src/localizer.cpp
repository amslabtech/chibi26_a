...
{ 
    ...
    // Subscriberの設定
    sub_map_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>("map", 10, std::bind(&SecondChallenge::scan_callback, this, std::placeholders::_1));
    sub_odom_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>("odom", 10, std::bind(&SecondChallenge::scan_callback, this, std::placeholders::_1));
    sub_laser_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>("laser", 10, std::bind(&SecondChallenge::scan_callback, this, std::placeholders::_1));

    // Publisherの設定
    pub_estimated_pose_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("cmd_vel", 10);
    pub_particle_cloud_ = this->create_publisher<geometry_msgs::msg::PoseArray>("cmd_vel", 10);
